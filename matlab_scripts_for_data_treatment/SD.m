function SD(scriptName)
    ...
    SDFiles = fileList(contains({fileList.name}, 'SD'));
    
    if isempty(SDFiles)
        warning('No file with "SD" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(SDFiles)
        fileName = SDFiles(fileIdx).name;
