function G1G2(scriptName)
    ...
    % Filter only files with "G1G2" in the name
    g1g2Files = fileList(contains({fileList.name}, 'G1G2'));
    
    if isempty(g1g2Files)
        warning('No file with "G1G2" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(g1g2Files)
        fileName = g1g2Files(fileIdx).name;
