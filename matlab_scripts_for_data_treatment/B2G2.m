function B2G2(scriptName)
    ...
    % Filter only files with "B2G2" in the name
    b2g2Files = fileList(contains({fileList.name}, 'B2G2'));
    
    if isempty(b2g2Files)
        warning('No file with "B2G2" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(b2g2Files)
        fileName = b2g2Files(fileIdx).name;
