function modulationG2(scriptName)
    fprintf('Running %s\n', scriptName);
    ...
    modulationG2Files = fileList(contains({fileList.name}, 'modulationG2'));
    
    if isempty(modulationG2Files)
        warning('No file with "modulationG2" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(modulationG2Files)
        fileName = modulationG2Files(fileIdx).name;
        ...
        for col = 2:size(data, 2)
            ...
            title(sprintf('Vg2-%s', string(units{col})));
            grid on;
    ...
    disp('ModulationG2 plots complete.');
