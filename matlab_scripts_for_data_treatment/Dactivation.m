function Dactivation(scriptName)
    fprintf('Running %s\n', scriptName);
    ...
    DaFiles = fileList(contains({fileList.name}, 'Dactivation'));
    
    if isempty(DaFiles)
        warning('No file with "Dactivation" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(DaFiles)
        fileName = DaFiles(fileIdx).name;
    ...
    disp('Drain activation plot complete.');
