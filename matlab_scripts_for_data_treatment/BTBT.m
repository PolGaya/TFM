...
function BTBT(scriptName)
    ...
    % Filter only files with "BTBT" in the name
    btbtFiles = fileList(contains({fileList.name}, 'BTBT'));

    if isempty(btbtFiles)
        warning('No file with "BTBT" in the name was found.');
        return;
    end

    % Loop through each matching Excel file
    for fileIdx = 1:length(btbtFiles)
        fileName = btbtFiles(fileIdx).name;
        ...
        xlabel('V_ test structure(V)');
        ylabel('I(A)');
        title('B_T-B_T');
        legend('I_ (pad1)', 'I_ (pad2)');
        grid on;
