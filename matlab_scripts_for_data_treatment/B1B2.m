function B1B2(scriptName)
    ...
    % Filter only files with "B1B2" in the name
    b1b2Files = fileList(contains({fileList.name}, 'B1B2'));
    
    if isempty(b1b2Files)
        warning('No file with "B1B2" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(b1b2Files)
        fileName = b1b2Files(fileIdx).name;
        ...
        ylabel('I(A)');
        title('B_1-B_2');
        legend('I_ (barrier1)', 'I_ (barrier2)');
        grid on;
