function modulationG1(scriptName)
    fprintf('Running %s\n', scriptName);
    ...
    modulationG1Files = fileList(contains({fileList.name}, 'modulationG1'));
    
    if isempty(modulationG1Files)
        warning('No file with "modulationG1" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(modulationG1Files)
        fileName = modulationG1Files(fileIdx).name;
    ...
    disp('ModulationG1 plots complete.');
