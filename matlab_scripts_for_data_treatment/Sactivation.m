function Sactivation(scriptName)
    fprintf('Running %s\n', scriptName);
    ...
    % Filter only files with "activacioS" in the name 
    SaFiles = fileList(contains({fileList.name}, 'Sactivation'));
    
    if isempty(SaFiles)
        warning('No file with "Sactivation" in the name was found.');
        return;
    end
    
    % Loop through each matching Excel file
    for fileIdx = 1:length(SaFiles)
        fileName = SaFiles(fileIdx).name;
    ...
    disp('Source activation plot complete.');
